...
export function useSidebarController({
  ...
  const loadMoreSessions = useCallback(
    async (project: Project) => {
      ...
      try {
        const currentSessionCount = IS_CODEX_ONLY_HARDENED
          ? getProjectSessions(project).length
          : (project.sessions?.length || 0) + (additionalSessions[project.name]?.length || 0);
        const projectPath = project.fullPath || project.path;

        if (IS_CODEX_ONLY_HARDENED && !projectPath) {
          return;
        }

        const response = IS_CODEX_ONLY_HARDENED
          ? await api.codexSessions(projectPath as string, 5, currentSessionCount)
          : await api.sessions(project.name, 5, currentSessionCount);
        ...
        setAdditionalSessions((prev) => {
          ...
          const seenIds = new Set(existing.map((session) => session.id).filter(Boolean));
    ...
    [additionalSessions, getProjectSessions, loadingSessions, projectHasMoreOverrides],
  );
  ...
  const refreshProjects = useCallback(async () => {
    ...
    try {
      if (IS_CODEX_ONLY_HARDENED) {
        // Hardened mode treats the server payload as the source of truth across devices.
        setAdditionalSessions({});
        setProjectHasMoreOverrides({});
      }
