...
export default function ChatComposer({
  ...
  const hasQuestionPanel = pendingPermissionRequests.some(
    (request) => request.toolName === 'AskUserQuestion' || request.resolutionMode === 'question'
  );
  ...
  return (
    <div className={`flex-shrink-0 p-2 pb-2 sm:p-4 sm:pb-4 md:p-4 md:pb-6 ${mobileFloatingClass}`}>
      ...
      <div className="mx-auto mb-3 max-w-4xl">
        ...
        {isInputBlocked && (
          <div className="mb-3 rounded-xl border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-100">
            {inputBlockedReason}
          </div>
        )}
