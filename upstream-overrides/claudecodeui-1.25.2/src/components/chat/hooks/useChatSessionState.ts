...
import {
  ...
  getLegacyChatMessagesStorageKey,
  safeLocalStorage,
...
export function useChatSessionState({
  ...
  const lastLoadedProjectRef = useRef<Project | null>(null);
  const lastLoadedSessionRef = useRef<ProjectSession | null>(null);
  const lastChatCacheStateRef = useRef<string | null>(null);
  ...
  const chatMessagesStorageKey = useMemo(
    () => getChatMessagesStorageKey(selectedProject?.name, selectedSession?.id),
  ...
  const loadSessionMessages = useCallback(
    async (projectName: string, sessionId: string, loadMore = false, provider: Provider | string = 'claude') => {
      ...
      recordSessionSyncEvent({
        sessionId,
        event: 'session_messages_load_start',
        detail: {
          projectName,
          loadMore,
          provider,
      ...
      try {
        ...
        if (data.hasMore !== undefined) {
          ...
          recordSessionSyncEvent({
            sessionId,
            event: 'session_messages_load_success',
            detail: {
              projectName,
              loadMore,
              provider,
        ...
        recordSessionSyncEvent({
          sessionId,
          event: 'session_messages_load_error',
          detail: {
            projectName,
            loadMore,
            provider,
  ...
  const forceSessionSyncRef = useRef(false);

  const applySessionMessages = useCallback((messages: any[], options: { forceChatSync?: boolean } = {}) => {
    if (options.forceChatSync) {
      forceSessionSyncRef.current = true;
    }
    setSessionMessages(messages);
  }, []);

  useEffect(() => {
    const legacyKey = getLegacyChatMessagesStorageKey(selectedProject?.name);
    if (legacyKey) {
      safeLocalStorage.removeItem(legacyKey);
    }

    if (!chatMessagesStorageKey) {
      setChatMessages([]);
      lastChatCacheStateRef.current = 'empty-key';
      return;
    }

    const saved = safeLocalStorage.getItem(chatMessagesStorageKey);
    if (!saved) {
      setChatMessages([]);
      lastChatCacheStateRef.current = 'miss';
      recordSessionSyncEvent({
        sessionId: selectedSession?.id ?? null,
        event: 'chat_cache_miss',
        detail: {
          storageKey: chatMessagesStorageKey,
        },
      });
      return;
    }
    ...
    try {
      setChatMessages(JSON.parse(saved) as ChatMessage[]);
      lastChatCacheStateRef.current = 'hit';
      recordSessionSyncEvent({
        sessionId: selectedSession?.id ?? null,
      ...
      safeLocalStorage.removeItem(chatMessagesStorageKey);
      setChatMessages([]);
      lastChatCacheStateRef.current = 'parse_error';
      recordSessionSyncEvent({
        sessionId: selectedSession?.id ?? null,
        event: 'chat_cache_parse_error',
        detail: {
          storageKey: chatMessagesStorageKey,
        },
      });
    }
  }, [chatMessagesStorageKey, selectedProject?.name, selectedSession?.id]);
    ...
    const loadMessages = async () => {
      ...
      if (selectedSession && selectedProject) {
        ...
        recordSessionSyncEvent({
          ...
          event: 'session_view_load_start',
          detail: {
            requestId,
            projectName: selectedProject.name,
            provider,
            selectedSessionRefreshKey,
          },
        });
        ...
        if (
          ...
          lastLoadedProjectRef.current === selectedProject &&
          lastLoadedSessionRef.current === selectedSession
        ) {
          recordSessionSyncEvent({
            sessionId: selectedSession.id,
            event: 'session_view_load_skipped',
            detail: {
              requestId,
              reason: 'same-session-key',
            },
          });
          setTimeout(() => {
        ...
        if (provider === 'cursor') {
          ...
          if (!isSystemSessionChange) {
            ...
            const converted = await loadCursorSessionMessages(projectPath, requestSessionId);
          ...
          if (!isSystemSessionChange) {
            ...
            applySessionMessages(messages, { forceChatSync: true });
          } else {
            setIsSystemSessionChange(false);
          }
        }
        ...
        recordSessionSyncEvent({
          sessionId: selectedSession.id,
          event: 'session_view_load_applied',
          detail: {
            requestId,
            projectName: selectedProject.name,
            provider,
  ...
  useEffect(() => {
    ...
    const reloadExternalMessages = async () => {
      const requestId = ++externalReloadRequestIdRef.current;
      recordSessionSyncEvent({
        sessionId: selectedSession.id,
        event: 'session_reload_start',
        detail: {
          requestId,
      ...
      try {
        ...
        if (provider === 'cursor') {
          ...
          if (externalReloadRequestIdRef.current !== requestId) {
            return;
          }
          setSessionMessages([]);
          setChatMessages(converted);
  ...
  useEffect(() => {
    ...
    const shouldForceSync = forceSessionSyncRef.current;
    if (
      (sessionMessages.length !== prevSessionMessagesLengthRef.current || shouldForceSync) &&
      (!isLoading || shouldForceSync)
    ) {
      ...
      forceSessionSyncRef.current = false;
    }
  ...
  useEffect(() => {
    ...
    const serverMessageCount = sessionMessages.length;
    const renderedMessageCount = chatMessages.length;
    const latestMessageTimestamp = sessionMessages.length > 0
      ? String(sessionMessages[sessionMessages.length - 1]?.timestamp || '')
      : null;

    updateSessionSyncSnapshot({
      sessionId: activeSessionId,
      selectedProject: selectedProject?.name ?? null,
      selectedSession: selectedSession?.id ?? currentSessionId ?? null,
      selectedSessionRefreshKey,
      externalMessageUpdate: externalMessageUpdate ?? null,
      reloadReason: externalMessageUpdate ? 'externalMessageUpdate' : null,
      chatCache: lastChatCacheStateRef.current,
      serverMessageCount,
    ...
    if (lastSyncStateRef.current === syncStateKey) {
      return;
    }

    lastSyncStateRef.current = syncStateKey;
    recordSessionSyncEvent({
      sessionId: activeSessionId,
      event: serverMessageCount === renderedMessageCount
        ? 'session_sync_aligned'
        : 'sync_divergence_detected',
      detail: {
        selectedProject: selectedProject?.name ?? null,
        selectedSessionRefreshKey,
        chatCache: lastChatCacheStateRef.current,
  ...
  useEffect(() => {
    ...
    if (chatMessages.length > 0) {
      safeLocalStorage.setItem(chatMessagesStorageKey, JSON.stringify(chatMessages));
    } else {
      clearChatMessagesCache(selectedProject?.name, selectedSession?.id);
    }
  }, [chatMessages, chatMessagesStorageKey, currentSessionId, selectedProject?.name, selectedSession?.id]);
        ...
        if (sessionProvider !== 'cursor') {
          try {
            ...
            if (response.ok) {
              ...
              applySessionMessages(Array.isArray(allMessages) ? allMessages : [], { forceChatSync: true });
              setHasMoreMessages(false);
  ...
  }, [applySessionMessages, chatMessages.length, isLoadingSessionMessages, searchTarget]);
  ...
  useEffect(() => {
    ...
    if (terminalSessions?.has(activeViewSessionId)) {
      return;
    }
  ...
  }, [currentSessionId, isLoading, processingSessions, selectedSession?.id, terminalSessions]);
  ...
  }, [selectedSession, selectedProject, isLoadingAllMessages, currentSessionId, applySessionMessages]);
