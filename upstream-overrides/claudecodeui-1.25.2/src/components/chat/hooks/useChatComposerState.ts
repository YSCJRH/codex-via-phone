...
export function useChatComposerState({
  ...
  const handleSubmit = useCallback(
    async (
      ...
      if (provider === 'codex' && selectedSession?.id && selectedSession.resumeSupported === false) {
        const blockedReason = selectedSession.threadHealth === 'recovered'
          ? 'This is a recovered Codex thread. Sending from mobile would create a new thread instead of updating this desktop thread.'
          : 'This Codex thread cannot be resumed safely right now.';
        setChatMessages((previous) => [
          ...previous,
          {
            type: 'error',
      ...
      if (!ws || ws.readyState !== WebSocket.OPEN) {
        setChatMessages((previous) => [
          ...
          {
            type: 'error',
            content: 'Mobile Codex is reconnecting. Your message was not sent. Please wait a moment and try again.',
            timestamp: new Date(),
          },
        ]);
        setIsLoading(false);
    ...
    [
      ...
      selectedProject,
      selectedSession?.id,
      selectedSession?.resumeSupported,
      selectedSession?.threadHealth,
      ws,
      sendMessage,
      setCanAbortSession,
      setChatMessages,
