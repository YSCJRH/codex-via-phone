...
import { clearChatMessagesCache } from '../utils/chatStorage';
...
interface UseChatRealtimeHandlersArgs {
  ...
  onSessionTerminal?: (sessionId?: string | null) => void;
  terminalSessions?: Set<string>;
  onReplaceTemporarySession?: (sessionId?: string | null) => void;
  onNavigateToSession?: (sessionId: string) => void;
  onCatchUpSessionMessages?: (sessionId?: string | boolean | null) => void;
...
const normalizePendingRequest = (
  value: unknown,
  fallback: Partial<PendingPermissionRequest> = {},
  ...
  const requestId = String(payload.interactionId || payload.requestId || fallback.interactionId || fallback.requestId || '').trim();
  if (!requestId) {
    return null;
  }
  ...
  return {
    ...
    provider:
      typeof payload.provider === 'string'
        ? (payload.provider as PendingPermissionRequest['provider'])
        : (fallback.provider ?? 'unknown'),
    kind: typeof payload.kind === 'string' ? payload.kind : (fallback.kind ?? 'permission'),
    toolName:
      typeof payload.toolName === 'string' && payload.toolName.trim().length > 0
        ? payload.toolName
        : (fallback.toolName ?? 'UnknownTool'),
    title: typeof payload.title === 'string' ? payload.title : (fallback.title ?? null),
    message: typeof payload.message === 'string' ? payload.message : (fallback.message ?? null),
    input: payload.input ?? fallback.input,
    context: payload.context ?? fallback.context,
    sessionId:
      typeof payload.sessionId === 'string'
        ? payload.sessionId
        : (fallback.sessionId ?? null),
    resolutionMode:
      payload.resolutionMode === 'question' || payload.resolutionMode === 'desktop-only'
        ? payload.resolutionMode
        : (fallback.resolutionMode ?? 'approve-deny'),
    options: Array.isArray(payload.options)
      ? (payload.options as PendingPermissionRequest['options'])
      : (fallback.options ?? []),
    metadata:
    ...
    receivedAt:
      typeof payload.receivedAt === 'string' || payload.receivedAt instanceof Date
        ? payload.receivedAt
        : (fallback.receivedAt ?? new Date()),
  };
};

const sortPendingRequests = (items: PendingPermissionRequest[]) => (
...
const mergePendingRequests = (
  previous: PendingPermissionRequest[],
  incoming: PendingPermissionRequest[],
  replaceSessionId?: string | null,
) => {
  const base =
    typeof replaceSessionId === 'string'
      ? previous.filter((request) => request.sessionId !== replaceSessionId)
      : [...previous];

  const merged = new Map(base.map((request) => [request.requestId, request]));
  incoming.forEach((request) => {
    const existing = merged.get(request.requestId);
    merged.set(request.requestId, existing ? { ...existing, ...request } : request);
  });

  return sortPendingRequests(Array.from(merged.values()));
};
...
export function useChatRealtimeHandlers({
  ...
  useEffect(() => {
    ...
    const activeViewSessionIds = collectSessionIds(
      selectedSession?.id,
      currentSessionId,
      pendingViewSessionRef.current?.sessionId,
      pendingSessionId,
    );
    const activeViewSessionId = activeViewSessionIds[0] || null;
    ...
    const isSystemInitForView =
      systemInitSessionId && (!activeViewSessionIds.length || activeViewSessionIds.includes(systemInitSessionId));
    const shouldBypassSessionFilter = isGlobalMessage || Boolean(isSystemInitForView);
    const latestMessageMatchesActiveView =
      typeof latestMessage.sessionId === 'string' && activeViewSessionIds.includes(latestMessage.sessionId);
    ...
    const appendPendingRequest = (payload: unknown, fallback: Partial<PendingPermissionRequest> = {}) => {
      const nextRequest = normalizePendingRequest(payload, fallback);
      if (!nextRequest) {
        return;
      }

      setPendingPermissionRequests((previous) => mergePendingRequests(previous, [nextRequest]));
    };

    const replacePendingSnapshot = (sessionId: string | null | undefined, snapshot: unknown) => {
      const normalizedRequests = Array.isArray(snapshot)
        ? snapshot
            .map((item) => normalizePendingRequest(item, { sessionId: sessionId ?? null }))
            .filter((item): item is PendingPermissionRequest => Boolean(item))
        : [];

      setPendingPermissionRequests((previous) =>
        mergePendingRequests(previous, normalizedRequests, sessionId ?? undefined),
      );
    };
    ...
    if (!shouldBypassSessionFilter) {
      ...
      if (latestMessage.sessionId && !latestMessageMatchesActiveView) {
    ...
    switch (latestMessage.type) {
      case 'session-created': {
        const createdSessionId =
          typeof latestMessage.sessionId === 'string' && latestMessage.sessionId.length > 0
            ? latestMessage.sessionId
            : null;
        const previousSessionId =
          typeof latestMessage.previousSessionId === 'string' && latestMessage.previousSessionId.length > 0
            ? latestMessage.previousSessionId
            : null;
        const shouldReplaceBrokenSession = Boolean(
          createdSessionId
          && previousSessionId
          && (
            previousSessionId === currentSessionId
            || previousSessionId === selectedSession?.id
            || previousSessionId === pendingViewSessionRef.current?.sessionId
          ),
        );

        if (createdSessionId && (!currentSessionId || shouldReplaceBrokenSession)) {
          ...
          if (shouldReplaceBrokenSession) {
            onNavigateToSession?.(createdSessionId);
            setChatMessages((previous) => [
              ...previous,
              {
                type: 'assistant',
                content: 'The previous Codex thread could not be resumed, so this message is continuing in a new thread.',
                timestamp: new Date(),
              },
            ]);
          }
          ...
          setPendingPermissionRequests((previous) =>
            previous.map((request) => {
              if (!request.sessionId || request.sessionId === previousSessionId) {
                return { ...request, sessionId: createdSessionId };
              }
              return request;
            }),
          );
      ...
      case 'claude-permission-request':
        ...
        appendPendingRequest(latestMessage, {
          provider: 'claude',
          kind: 'permission',
          title: 'Permission required',
          resolutionMode: 'approve-deny',
          receivedAt: new Date(),
        });
      ...
      case 'interaction-required':
        appendPendingRequest(latestMessage.data ?? latestMessage, {
          sessionId: latestMessage.sessionId || selectedSession?.id || currentSessionId || null,
          receivedAt: new Date(),
        });
        setIsLoading(true);
        setCanAbortSession(true);
      ...
      case 'claude-complete': {
        ...
        if (selectedProject && latestMessage.exitCode === 0) {
          clearChatMessagesCache(selectedProject.name, completedSessionId);
        }
        break;
      }
      ...
      case 'codex-response': {
        ...
        if (codexData.type === 'turn_complete') {
          finalizeLifecycleForCurrentView(latestMessage.sessionId, currentSessionId, selectedSession?.id);
          onCatchUpSessionMessages?.(true);
        }

        if (codexData.type === 'task_complete') {
          finalizeLifecycleForCurrentView(latestMessage.sessionId, currentSessionId, selectedSession?.id);
          onCatchUpSessionMessages?.(true);
        }
      ...
      case 'codex-complete': {
        ...
        if (selectedProject) {
          clearChatMessagesCache(
            ...
            codexActualSessionId || codexCompletedSessionId || selectedSession?.id || currentSessionId,
          );
        }
        onCatchUpSessionMessages?.(true);
        break;
      ...
      case 'session-aborted': {
        ...
        const finalStatus = typeof latestMessage.finalStatus === 'string' ? latestMessage.finalStatus : null;
      ...
      case 'session-status': {
        ...
        const isCurrentSession = activeViewSessionIds.includes(statusSessionId);
        const isTerminalSession = terminalSessions?.has(statusSessionId);
        ...
        onSessionTerminal?.(statusSessionId);
        if (isCurrentSession) {
          clearLoadingIndicators();
          if (isLoading) {
            onCatchUpSessionMessages?.(true);
      ...
      case 'pending-permissions-response': {
        ...
        replacePendingSnapshot(permSessionId, latestMessage.data || []);
        break;
      }

      case 'pending-interactions-response': {
        const interactionSessionId = latestMessage.sessionId;
        const isCurrentInteractionSession =
          interactionSessionId === currentSessionId || (selectedSession && interactionSessionId === selectedSession.id);
        if (interactionSessionId && !isCurrentInteractionSession) {
          break;
        }
        replacePendingSnapshot(interactionSessionId, latestMessage.data || []);
        break;
      }
