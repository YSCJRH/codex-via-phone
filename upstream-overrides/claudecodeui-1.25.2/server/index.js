...
import { getProjects, warmProjectsCache, getSessions, getSessionMessages, renameProject, deleteSession, deleteProject, addProjectManually, extractProjectDirectory, clearProjectDirectoryCache, searchConversations, compactProjectsForClient, resolveCodexSessionFilePath } from './projects.js';
import { queryClaudeSDK, abortClaudeSDKSession, isClaudeSDKSessionActive, getActiveClaudeSDKSessions, resolveToolApproval, reconnectSessionWriter } from './claude-sdk.js';
import { spawnCursor, abortCursorSession, isCursorSessionActive, getActiveCursorSessions } from './cursor-cli.js';
import { queryCodex, abortCodexSession, isCodexSessionActive, getActiveCodexSessions, getActiveCodexSessionContext, reconnectCodexSessionWriter } from './openai-codex.js';
...
import {
    getPendingDesktopApprovals,
    startDesktopApprovalMonitor,
} from './desktop-approval-monitor.js';
import {
...
let cursorRoutes = null;

if (!CODEX_ONLY_HARDENED_MODE) {
    try {
        ({ default: cursorRoutes } = await import('./routes/cursor.js'));
    } catch (error) {
        throw new Error(`Cursor routes require optional sqlite dependencies: ${error.message}`);
    }
}
...
const PROVIDER_WATCH_PATHS = CODEX_ONLY_HARDENED_MODE
    ? [
        ...
        { provider: 'codex_state', rootPath: path.join(os.homedir(), '.codex', 'state*.sqlite*') }
    ]
    : [
...
const CHAT_WS_HEARTBEAT_INTERVAL_MS = 15000;
let projectsWatchers = [];
let projectsWatcherDebounceTimer = null;
const connectedClients = new Set();
let isGetProjectsRunning = false; // Flag to prevent reentrant calls

function shouldHandleProjectWatchEvent(provider, rootPath, targetPath) {
    if (provider !== 'codex_state') {
        return true;
    }

    const relativePath = path.relative(rootPath, targetPath).replace(/\\/g, '/');
    return /^state(?:_\d+)?\.sqlite(?:-(?:wal|shm))?$/i.test(relativePath);
}
...
function extractSessionIdFromChangedFile(filePath) {
    ...
    return trailingUuidMatch ? trailingUuidMatch[1] : (basename || null);
}

function shouldTraceConnectivityRequest(req) {
    const requestPath = req.path || req.url || '';

    return requestPath === '/health'
        || requestPath.startsWith('/api/auth/')
        || requestPath === '/api/projects'
        || /^\/api\/codex\/sessions\/[^/]+(?:\/messages)?$/i.test(requestPath)
        || requestPath.startsWith('/api/debug/');
}

function extractSessionIdFromRequestPath(requestPath) {
    const match = String(requestPath || '').match(/^\/api\/codex\/sessions\/([^/]+)/i);
...
function broadcastDesktopApprovals(approvals) {
    const message = JSON.stringify({
        type: 'desktop-approvals-updated',
        data: approvals,
    });
...
async function sendDesktopApprovalsSnapshot(writer, options = {}) {
    const approvals = await getPendingDesktopApprovals(options);
    writer.send({
        type: 'desktop-approvals-response',
        data: approvals,
    });
}
...
async function setupProjectsWatcher() {
    ...
    const debouncedUpdate = (eventType, filePath, provider, rootPath) => {
        if (!shouldHandleProjectWatchEvent(provider, rootPath, filePath)) {
            return;
        }
        ...
        projectsWatcherDebounceTimer = setTimeout(async () => {
            ...
            try {
                ...
                const clientProjects = compactProjectsForClient(updatedProjects);
                ...
                void recordSyncEvent({
                    surface: 'server',
                    sessionId: extractSessionIdFromChangedFile(path.relative(rootPath, filePath)),
                    event: 'projects_updated_broadcast',
                    detail: {
                        changeType: eventType,
                        changedFile: path.relative(rootPath, filePath),
                        watchProvider: provider,
...
function isLoopbackAddress(address) {
    ...
    const normalizedAddress = address.replace(/^\[|\]$/g, '').replace(/^::ffff:/, '').toLowerCase();
...
const corsOptionsDelegate = (req, callback) => {
    ...
    console.warn('[WARN] Blocked CORS origin:', origin);
    return callback(null, {
        credentials: true,
        origin: false,
        optionsSuccessStatus: 204
...
app.use((req, res, next) => {
    if (!shouldTraceConnectivityRequest(req)) {
        return next();
    }
    ...
    res.on('finish', () => {
        void recordConnectivityEvent({
            surface: 'server',
            sessionId: extractSessionIdFromRequestPath(req.path || req.url || ''),
            event: 'http_request',
            detail: {
                method: req.method,
...
// Debug diagnostics API Routes (protected)
...
app.use(express.static(path.join(__dirname, '../public'), {
    setHeaders: (res, filePath) => {
        if (
            ...
            filePath.endsWith('clear-cache.html') ||
...
app.get('/api/projects', authenticateToken, async (req, res) => {
    try {
        const projects = CODEX_ONLY_HARDENED_MODE
            ? await getProjects(null, { preferCache: true, refreshInBackground: true })
            : await getProjects(broadcastProgress);
        res.json(compactProjectsForClient(projects));
    } catch (error) {
...
function handleChatConnection(ws, request = null) {
    ...
    const connectionId = ws.connectionId || request?.connectionId || null;
    void recordConnectivityEvent({
        surface: 'server',
        connectionId,
    ...
    let heartbeatTimer = setInterval(() => {
        ...
        if (ws.isAlive === false) {
            console.warn('[WARN] Terminating stale chat WebSocket client');
            void recordConnectivityEvent({
                surface: 'server',
                connectionId,
                event: 'ws_stale_terminate',
                detail: null,
            });
        ...
        ws.ping();
    }, CHAT_WS_HEARTBEAT_INTERVAL_MS);

    // Wrap WebSocket with writer for consistent interface with SSEStreamWriter
    const writer = new WebSocketWriter(ws);
    void sendDesktopApprovalsSnapshot(writer);
    ...
    ws.on('message', async (message) => {
        try {
            ...
            if (CODEX_ONLY_HARDENED_MODE) {
                ...
                const sessionRecoveryMessageTypes = new Set([
                    'get-pending-permissions',
                    'get-pending-interactions',
                    'interaction-response'
                ]);
                const isCodexProviderMessage =
                    data.type === 'codex-command'
                    || provider === 'codex'
                    || sessionRecoveryMessageTypes.has(data.type);
            ...
            if (data.type === 'claude-command') {
                ...
                if (provider === 'cursor') {
                    ...
                    codexContext = getActiveCodexSessionContext(data.sessionId);
                ...
                writer.send({
                    ...
                    provider,
                    success,
                    finalStatus: codexContext?.status || null,
                    completionSource: codexContext?.completionSource || null,
                });
                ...
                if (data.requestId) {
                    const interaction = getPendingInteraction(data.requestId);
                    if (interaction?.provider === 'claude') {
                        ...
                        resolvePendingInteraction(data.requestId, {
                            allow: Boolean(data.allow),
                            updatedInput: data.updatedInput,
                            message: data.message,
                            rememberEntry: data.rememberEntry
            ...
            } else if (data.type === 'interaction-response') {
                ...
                if (interactionId) {
                    ...
                    if (interaction?.provider === 'claude') {
                        resolveToolApproval(interactionId, decision);
                    } else {
                        resolvePendingInteraction(interactionId, decision);
                    }
                }
                ...
                if (provider === 'cursor') {
                    ...
                    codexContext = getActiveCodexSessionContext(sessionId);
                    if (isActive) {
                ...
                const sessionId = data.sessionId;
                if (sessionId) {
                    const pending = listPendingInteractionsForSession(sessionId);
                    writer.send({
            ...
            } else if (data.type === 'get-pending-interactions') {
                const sessionId = data.sessionId;
                if (sessionId) {
                    writer.send({
                        type: 'pending-interactions-response',
                        sessionId,
                        data: listPendingInteractionsForSession(sessionId)
                    });
                }
            } else if (data.type === 'get-desktop-approvals') {
                await sendDesktopApprovalsSnapshot(writer, { refresh: true });
...
async function startServer() {
    try {
        ...
        const stopDesktopApprovalMonitor = startDesktopApprovalMonitor(broadcastDesktopApprovals);
        ...
        server.listen(PORT, HOST, async () => {
            ...
            if (CODEX_ONLY_HARDENED_MODE) {
                warmProjectsCache().catch((error) => {
                    console.error('[WARN] Failed to prewarm Codex project cache:', error);
                });
            }
