...
import {
  ...
  normalizeComparablePath as normalizeComparableProjectPath,
  resolveProjectPath as resolveProjectPathValue,
...
let cursorSqliteModulesPromise = null;
let codexSqliteModulePromise = null;
let codexStateDbPathPromise = null;

async function getCursorSqliteModules() {
  if (!cursorSqliteModulesPromise) {
    cursorSqliteModulesPromise = Promise.all([
      import('sqlite3'),
      import('sqlite')
  ...
  return cursorSqliteModulesPromise;
}

async function getCodexSqliteModule() {
  if (!codexSqliteModulePromise) {
    codexSqliteModulePromise = import('better-sqlite3')
      .then((module) => module.default ?? module)
      .catch((error) => {
        console.warn('Codex SQLite support unavailable, falling back to session scan:', error.message);
        return null;
      });
  }

  return codexSqliteModulePromise;
}

async function resolveCodexStateDbPath() {
  if (!codexStateDbPathPromise) {
    codexStateDbPathPromise = (async () => {
      ...
      for (const entry of entries) {
        ...
        try {
          ...
          candidates.push({
            fullPath,
            version: Number.parseInt(match[1] ?? '0', 10) || 0,
  ...
  return codexStateDbPathPromise;
}
...
// Cache for extracted project directories and project snapshots
const projectDirectoryCache = new Map();
const PROJECTS_CACHE_DIR = process.env.DATABASE_PATH
  ? path.join(path.dirname(path.resolve(process.env.DATABASE_PATH)), 'cache')
  : path.join(os.homedir(), '.codex', 'mobile-codex-helper-cache');
const PROJECTS_CACHE_FILE = path.join(
  PROJECTS_CACHE_DIR,
  CODEX_ONLY_HARDENED_MODE ? 'projects-codex-only.json' : 'projects-full.json'
);
const PROJECTS_CACHE_VERSION = PROJECTS_CACHE_SCHEMA_VERSION;
const PROJECTS_CACHE_STALE_MS = 30 * 1000;
const CODEX_PROJECTS_PAGE_SIZE = 5;
let projectsSnapshotCache = null;
let projectsSnapshotUpdatedAt = 0;
let projectsSnapshotLoadedFromDisk = false;
let projectsSnapshotRefreshPromise = null;
...
function dedupeCodexSessions(sessions) {
  ...
  return [...deduped.values()].sort((a, b) => getSessionTimestamp(b) - getSessionTimestamp(a));
}

function normalizeCodexOnlyProject(project, limit = CODEX_PROJECTS_PAGE_SIZE) {
  if (!project || !CODEX_ONLY_HARDENED_MODE) {
    return project;
  }

  const codexSessions = dedupeCodexSessions(project.codexSessions);
  const total = Math.max(Number(project.sessionMeta?.total) || 0, codexSessions.length);
  const effectiveLimit = limit > 0 ? limit : codexSessions.length;

  return {
    ...project,
    codexSessions: codexSessions.slice(0, effectiveLimit),
    sessionMeta: {
      ...(project.sessionMeta || {}),
      total,
      hasMore: total > effectiveLimit,
    },
  };
}

function normalizeProjectsSnapshot(projects, limit = CODEX_PROJECTS_PAGE_SIZE) {
  if (!Array.isArray(projects)) {
    return [];
  }

  return projects.map((project) => normalizeCodexOnlyProject(project, limit));
}

function markProjectsSnapshotStale() {
  projectsSnapshotUpdatedAt = 0;
}

async function ensureProjectsSnapshotLoaded() {
  if (projectsSnapshotLoadedFromDisk || !CODEX_ONLY_HARDENED_MODE) {
    return;
  }

  projectsSnapshotLoadedFromDisk = true;

  try {
    const raw = await fs.readFile(PROJECTS_CACHE_FILE, 'utf8');
    const payload = JSON.parse(raw);
    if (payload?.version !== PROJECTS_CACHE_VERSION) {
      return;
    }
    ...
    projectsSnapshotCache = cloneProjectsSnapshot(normalizeProjectsSnapshot(projects));
    projectsSnapshotUpdatedAt =
      Number.isFinite(payload?.updatedAt) && payload.updatedAt > 0
        ? payload.updatedAt
        : Date.now();
  } catch (error) {
    if (error?.code !== 'ENOENT') {
      console.warn('Failed to load cached project snapshot:', error.message);
    }
  }
}

async function persistProjectsSnapshot(projects) {
  if (!CODEX_ONLY_HARDENED_MODE) {
    return;
  }

  const snapshot = cloneProjectsSnapshot(normalizeProjectsSnapshot(projects));
  const updatedAt = Date.now();

  projectsSnapshotCache = snapshot;
  projectsSnapshotUpdatedAt = updatedAt;

  try {
    await fs.mkdir(PROJECTS_CACHE_DIR, { recursive: true });

    const tempPath = `${PROJECTS_CACHE_FILE}.tmp`;
    await fs.writeFile(
      tempPath,
      JSON.stringify(
        {
    ...
    await fs.rename(tempPath, PROJECTS_CACHE_FILE);
  } catch (error) {
    console.warn('Failed to persist cached project snapshot:', error.message);
  }
}

function getCachedProjectsSnapshot() {
  if (!Array.isArray(projectsSnapshotCache)) {
    return null;
  }

  return cloneProjectsSnapshot(normalizeProjectsSnapshot(projectsSnapshotCache));
}

function projectsSnapshotIsStale() {
  return !projectsSnapshotUpdatedAt || (Date.now() - projectsSnapshotUpdatedAt) > PROJECTS_CACHE_STALE_MS;
}

function getProjectsCacheDebugInfo() {
  return {
    cacheFile: PROJECTS_CACHE_FILE,
    schemaVersion: PROJECTS_CACHE_SCHEMA_VERSION,
    updatedAt: projectsSnapshotUpdatedAt ? new Date(projectsSnapshotUpdatedAt).toISOString() : null,
    updatedAtMs: projectsSnapshotUpdatedAt || 0,
    isStale: projectsSnapshotIsStale(),
  };
}
...
function toCodexOnlyProject(project) {
  ...
  return {
    ...
    geminiSessions: [],
    sessionMeta: {
      hasMore: codexSessions.length > CODEX_PROJECTS_PAGE_SIZE,
...
function clearProjectDirectoryCache() {
  ...
  markProjectsSnapshotStale();
}

function resolveProjectPath(inputPath) {
  return resolveProjectPathValue(inputPath);
}
...
async function getCodexOnlyProjects(progressCallback, config, codexSessionsIndexRef) {
  ...
  const projectEntries = Array.from(sessionsByProject.entries())
    .map(([normalizedProjectPath, sessions]) => {
    ...
    .filter(({ actualProjectDir, matchedMetadata, projectName }) => {
      if (!projectName || !actualProjectDir) {
        return false;
      }

      return !shouldHideAutoDetectedProject(actualProjectDir, {
  ...
  for (const { sessions, actualProjectDir, matchedMetadata, projectName } of projectEntries) {
    processedProjects++;
    ...
    const pagedSessions = sessions.slice(0, CODEX_PROJECTS_PAGE_SIZE);
    ...
    projects.push({
      ...
      sessions: [],
      cursorSessions: [],
      codexSessions: pagedSessions,
      geminiSessions: [],
      sessionMeta: {
        hasMore: sessions.length > CODEX_PROJECTS_PAGE_SIZE,
  ...
  const sortedProjects = sortCodexProjectsForDisplay(projects);
...
async function generateDisplayName(projectName, actualProjectDir = null, options = {}) {
  // Use actual project directory if provided, otherwise decode from project name
  let projectPath = actualProjectDir || projectName.replace(/-/g, '/');
  const shouldReadPackageJson = options.preferPackageJson ?? !CODEX_ONLY_HARDENED_MODE;
  ...
  return getProjectPathDisplayName(projectPath, projectPath);
}
...
async function computeProjects(progressCallback = null) {
  const config = await loadProjectConfig();
  ...
  try {
    ...
    for (const entry of directories) {
      ...
      if (!CODEX_ONLY_HARDENED_MODE) {
        // Cursor discovery is disabled in hardened mode to avoid loading optional sqlite code paths.
        try {
      ...
      try {
        const codexSessionResult = await getCodexSessionsPage(actualProjectDir, {
          limit: CODEX_PROJECTS_PAGE_SIZE,
          offset: 0,
          indexRef: codexSessionsIndexRef,
        });
        project.codexSessions = codexSessionResult.sessions;
        if (CODEX_ONLY_HARDENED_MODE) {
          project.sessionMeta = {
            hasMore: codexSessionResult.hasMore,
            total: codexSessionResult.total,
          };
        }
      } catch (e) {
...
function startProjectsSnapshotRefresh(progressCallback = null) {
  if (projectsSnapshotRefreshPromise) {
    return projectsSnapshotRefreshPromise;
  }

  projectsSnapshotRefreshPromise = (async () => {
    const projects = await computeProjects(progressCallback);
    await persistProjectsSnapshot(projects);
    return cloneProjectsSnapshot(projects);
  })().finally(() => {
    projectsSnapshotRefreshPromise = null;
  });

  return projectsSnapshotRefreshPromise;
}

async function warmProjectsCache(progressCallback = null) {
  if (!CODEX_ONLY_HARDENED_MODE) {
    return null;
  }

  await ensureProjectsSnapshotLoaded();
  return startProjectsSnapshotRefresh(progressCallback);
}

async function getProjects(progressCallback = null, options = {}) {
  const {
    preferCache = CODEX_ONLY_HARDENED_MODE,
    refreshInBackground = preferCache,
  } = options;

  if (!CODEX_ONLY_HARDENED_MODE) {
    return computeProjects(progressCallback);
  }

  await ensureProjectsSnapshotLoaded();

  if (preferCache) {
    const cachedSnapshot = getCachedProjectsSnapshot();
    if (cachedSnapshot) {
      if (refreshInBackground && projectsSnapshotIsStale()) {
        void startProjectsSnapshotRefresh(null).catch((error) => {
          console.warn('Background project snapshot refresh failed:', error.message);
        });
      }
...
function compactSessionForClient(session) {
  if (!session || typeof session !== 'object') {
    return session;
  }

  const compact = {};
  for (const field of CLIENT_SESSION_FIELDS) {
    if (session[field] !== undefined) {
      compact[field] = session[field];
    }
  }
...
function compactProjectForClient(project) {
  ...
  return {
    ...project,
    sessions: Array.isArray(project.sessions) ? project.sessions.map(compactSessionForClient) : project.sessions,
    cursorSessions: Array.isArray(project.cursorSessions) ? project.cursorSessions.map(compactSessionForClient) : project.cursorSessions,
    codexSessions: Array.isArray(project.codexSessions) ? project.codexSessions.map(compactSessionForClient) : project.codexSessions,
    geminiSessions: Array.isArray(project.geminiSessions) ? project.geminiSessions.map(compactSessionForClient) : project.geminiSessions,
  };
}

function compactProjectsForClient(projects) {
  if (!Array.isArray(projects)) {
    return [];
  }

  return projects.map(compactProjectForClient);
}
...
async function getCursorSessions(projectPath) {
  ...
  try {
    const { sqlite3, open } = await getCursorSqliteModules();
...
function normalizeComparablePath(inputPath) {
  return normalizeComparableProjectPath(inputPath);
}
...
function normalizeCodexSummary(value, fallback = 'Codex Session') {
...
function unixTimestampToIsoString(value) {
...
async function getCodexThreadRowsFromStateDb() {
  const Database = await getCodexSqliteModule();
  ...
  try {
    ...
    console.warn(`Failed to read Codex state database ${dbPath}:`, error.message);
    return null;
  } finally {
    try {
      db?.close();
    } catch {
      // Ignore close failures from readonly handles.
    }
  }
}

async function getCodexThreadRowById(sessionId) {
  ...
  try {
    ...
    console.warn(`Failed to look up Codex thread ${safeSessionId} in state database ${dbPath}:`, error.message);
    return null;
  } finally {
    try {
      db?.close();
...
async function findCodexSessionFileById(sessionId) {
  ...
  return findSessionFile(codexSessionsDir);
}

async function resolveCodexSessionFilePath(sessionId) {
  const threadRow = await getCodexThreadRowById(sessionId);
  const candidatePath = typeof threadRow?.rollout_path === 'string' ? threadRow.rollout_path.trim() : '';
  ...
  if (candidatePath) {
    try {
      ...
      // Fall through to legacy session scan below.
    }
  }
...
async function buildCodexSessionFromThreadRow(threadRow) {
  if (!threadRow?.id || !threadRow.cwd) {
    return null;
  }

  let parsedSession = null;
  let rolloutExists = false;
  const rolloutPath = typeof threadRow.rollout_path === 'string' ? threadRow.rollout_path.trim() : '';
  const normalizedRolloutPath = rolloutPath.replace(/\\/g, '/').toLowerCase();
  const isRecoveredThread =
    normalizedRolloutPath.includes('/sessions/recovered/')
    || path.basename(rolloutPath).toLowerCase().startsWith('rollout-recovered-');

  if (rolloutPath) {
    try {
      await fs.access(rolloutPath);
      rolloutExists = true;
      parsedSession = await parseCodexSessionFile(rolloutPath);
    } catch {
      parsedSession = null;
    }
  }

  const threadHealth = isRecoveredThread
    ? 'recovered'
    : rolloutExists
      ? 'healthy'
      : 'missing-rollout';
  ...
  return {
    id: threadRow.id,
    title: normalizeCodexSummary(threadRow.title, 'Codex Session'),
    summary: normalizeCodexSummary(
      threadRow.title
        || threadRow.first_user_message
        || parsedSession?.summary,
      'Codex Session',
    ),
    messageCount: parsedSession?.messageCount || 0,
    createdAt: unixTimestampToIsoString(threadRow.created_at),
    lastActivity: unixTimestampToIsoString(threadRow.updated_at)
      || parsedSession?.timestamp
      || unixTimestampToIsoString(threadRow.created_at)
      || new Date().toISOString(),
    updated_at: unixTimestampToIsoString(threadRow.updated_at),
    cwd: resolveProjectPath(threadRow.cwd) || threadRow.cwd,
    model: threadRow.model || threadRow.model_provider || parsedSession?.model,
    filePath: rolloutPath || null,
    resumeSupported: rolloutExists && !isRecoveredThread,
    threadHealth,
    provider: 'codex',
  };
}
...
async function buildCodexSessionsIndex() {
  ...
  const threadRows = await getCodexThreadRowsFromStateDb();
  if (Array.isArray(threadRows) && threadRows.length > 0) {
    const sessions = [];

    for (const threadRow of threadRows) {
      try {
        const session = await buildCodexSessionFromThreadRow(threadRow);
        if (!session?.id) {
          continue;
        }
        ...
        console.warn(`Could not map Codex state thread ${threadRow?.id ?? 'unknown'}:`, error.message);
      }
    }

    return indexCodexSessionsByProjectPath(sessions);
  }
  ...
  return indexCodexSessionsByProjectPath(fallbackSessions);
}

async function getCodexSessionsPage(projectPath, options = {}) {
  const { limit = CODEX_PROJECTS_PAGE_SIZE, offset = 0, indexRef = null } = options;
  try {
    ...
    const pagedSessions = limit > 0 ? sessions.slice(offset, offset + limit) : sessions.slice(offset);

    return {
      sessions: pagedSessions,
      total: sessions.length,
      hasMore: offset + pagedSessions.length < sessions.length,
      offset,
      limit,
    };
  } catch (error) {
    console.error('Error fetching Codex sessions page:', error);
    return {
      sessions: [],
      total: 0,
      hasMore: false,
...
async function getCodexSessions(projectPath, options = {}) {
  const { sessions } = await getCodexSessionsPage(projectPath, options);
  return sessions;
}

async function resolveCodexSessionTarget(sessionId) {
  const safeSessionId = typeof sessionId === 'string' ? sessionId.trim() : '';
  if (!safeSessionId) {
    return null;
  }

  const threadRow = await getCodexThreadRowById(safeSessionId);
  if (!threadRow) {
    return null;
  }
  ...
  const actualProjectDir = resolveProjectPath(session.cwd) || resolveProjectPath(threadRow.cwd) || threadRow.cwd;
  const normalizedProjectPath = normalizeComparablePath(actualProjectDir);
  if (!actualProjectDir || !normalizedProjectPath) {
    return null;
  }
  ...
  try {
    const sessionPage = await getCodexSessionsPage(actualProjectDir, {
      limit: CODEX_PROJECTS_PAGE_SIZE,
      offset: 0,
    });
    codexSessions = dedupeCodexSessions([session, ...(sessionPage.sessions || [])]);
    totalSessions = Math.max(Number(sessionPage.total) || 0, codexSessions.length);
    hasMoreSessions = totalSessions > codexSessions.length;
  } catch (error) {
    console.warn(`Could not expand Codex session target ${safeSessionId}:`, error.message);
  }
...
async function getCodexSessionMessages(sessionId, limit = null, offset = 0) {
  try {
    const sessionFilePath = await resolveCodexSessionFilePath(sessionId);
