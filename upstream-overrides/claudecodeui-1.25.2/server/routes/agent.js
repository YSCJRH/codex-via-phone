...
const validateExternalApiKey = (req, res, next) => {
  ...
  // Self-hosted mode: Require API keys in headers so they do not leak via URLs or logs.
...
router.post('/', validateExternalApiKey, async (req, res) => {
  const { githubUrl, projectPath, message, provider = CODEX_ONLY_HARDENED_MODE ? 'codex' : 'claude', model, githubToken, branchName } = req.body;
  ...
  if (CODEX_ONLY_HARDENED_MODE && provider !== 'codex') {
    return res.status(403).json({ error: 'Only the codex provider is available in Codex-only hardened mode' });
  }
