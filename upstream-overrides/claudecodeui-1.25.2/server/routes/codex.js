...
import { getDesktopApprovalBridgeStatus, resolveDesktopApprovalRequest } from '../desktop-approval-bridge.js';
...
router.get('/sessions/:sessionId', async (req, res) => {
  try {
    const { sessionId } = req.params;
    const target = await resolveCodexSessionTarget(sessionId);

    if (!target?.project || !target?.session) {
      return res.status(404).json({ success: false, error: 'Codex session not found' });
    }

    const project = compactProjectsForClient([target.project])[0] || null;
    const session = project?.codexSessions?.find?.((entry) => entry.id === target.session.id) || null;

    if (!project || !session) {
      return res.status(404).json({ success: false, error: 'Codex session target unavailable' });
    }
...
router.post('/desktop-approvals/:requestId/resolve', async (req, res) => {
  try {
    const { requestId } = req.params;
    const { action, currentSessionId } = req.body || {};
    const result = await resolveDesktopApprovalRequest({
      requestId,
      action,
      viewerSessionId: currentSessionId,
      user: req.user || null,
      device: req.device || null,
    });

    res.status(result.httpStatus || (result.ok ? 200 : 409)).json({
      success: result.ok,
    ...
    res.status(error?.statusCode || 500).json({
      success: false,
      error: error?.message || 'Failed to resolve desktop approval',
      code: error?.code || 'desktop_approval_bridge_failed',
    });
  }
});

router.get('/desktop-approvals/bridge-status', async (_req, res) => {
  try {
    const status = await getDesktopApprovalBridgeStatus();
    res.json({
      success: true,
      status,
    });
    ...
    res.status(500).json({
      success: false,
      error: error?.message || 'Failed to read desktop approval bridge status',
      code: error?.code || 'desktop_approval_bridge_status_failed',
    });
  }
});
